...
// ActiveUserMiddleware rejects requests from disabled accounts (after JWT auth).
func ActiveUserMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		userID, err := utils.ParseID(GetUserID(c))
		if err != nil {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Invalid user"})
			c.Abort()
			return
		}
		row, err := s.Queries().GetUserByID(c.Request.Context(), userID)
		if err != nil {
