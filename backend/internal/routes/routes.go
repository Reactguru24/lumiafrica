...
func SetupRoutes(router *gin.Engine, st *store.Store, cfg *config.Config, rc *redis.Client) {
	...
	admin.Use(auth, activeUser, middleware.RoleMiddleware(models.RoleAdmin))
	{
		...
		admin.GET("/delivery-zones", handlers.ListAdminDeliveryZones())
		admin.POST("/delivery-zones", handlers.CreateAdminDeliveryZone())
		admin.PUT("/delivery-zones/:zoneID", handlers.UpdateAdminDeliveryZone())
		admin.DELETE("/delivery-zones/:zoneID", handlers.DeleteAdminDeliveryZone())
