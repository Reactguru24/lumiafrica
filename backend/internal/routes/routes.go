...
func SetupRoutes(router *gin.Engine, st *store.Store, cfg *config.Config, rc *redis.Client) {
	...
	admin.Use(auth, activeUser, middleware.RoleMiddleware(models.RoleAdmin))
	{
		...
		admin.GET("/shipping-lanes", handlers.ListAdminShippingLanes())
		admin.POST("/shipping-lanes", handlers.CreateAdminShippingLane())
		admin.PUT("/shipping-lanes/:laneID", handlers.UpdateAdminShippingLane())
		admin.DELETE("/shipping-lanes/:laneID", handlers.DeleteAdminShippingLane())
