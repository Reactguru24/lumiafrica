...
export default function ForgotPasswordPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const response = await forgotPassword.mutate(email) as { message?: string; resetToken?: string }
      setSent(true)
    ...
    <div>
      ...
      {sent ? (
        ...
        <form className="space-y-5" onSubmit={submit}>
          ...
          <button type="submit" className="btn-primary w-full" disabled={forgotPassword.loading}>
            {forgotPassword.loading ? 'Sending...' : 'Send Reset Instructions'}
          </button>
