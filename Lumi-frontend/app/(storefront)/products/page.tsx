...
import { useProducts, useProductFilters, useVendors, useVendor } from '@/lib/api/hooks'
...
function ProductsPageContent() {
  ...
  const queryParams = useMemo(() => {
    ...
    params.page = String(queryObj.page ? Number(queryObj.page) : 1)
    params.limit = '12'
    return params
  }, [filters, queryObj.page])

  const { data: productsData, loading, error } = useProducts(queryParams)
  ...
  function clearVendorFilter() {
    updateFilters({ ...filters, vendorId: undefined })
  }
    ...
    <div className="page-container">
      {filters.vendorId && (
        <div className="card p-4 mb-6 flex flex-col sm:flex-row sm:items-center gap-4 border-brand-teal/30 dark:border-brand-orange/30 bg-brand-teal/5 dark:bg-brand-orange/5">
          {activeVendor?.logo && (
            <Image
              src={resolveMediaUrl(activeVendor.logo)}
              alt={activeVendor.storeName || 'Vendor'}
              width={56}
              height={56}
              className="w-14 h-14 rounded-full object-cover shrink-0"
              unoptimized={(activeVendor.logo || '').startsWith('/uploads')}
            />
          )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="min-w-0">
          ...
          <p className="text-gray-500 text-sm mt-1">{loading ? 'Loading products...' : `${total} products`}</p>
        </div>
