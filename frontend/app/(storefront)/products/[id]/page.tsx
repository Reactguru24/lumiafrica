...
export default function ProductDetailPage() {
  ...
  if (loading) return (<div className="page-container flex items-center justify-center min-h-[40vh]"><div className="card p-8 text-center"><div className="skeleton h-8 w-48 mx-auto mb-3" /><p className="text-gray-500 animate-pulse">Loading product...</p></div></div>)
  if (error) return (<div className="page-container flex items-center justify-center min-h-[40vh]"><div className="card p-6 sm:p-8 text-center max-w-md"><p className="text-red-600 font-medium text-lg">Unable to load this product</p><p className="text-gray-500 mt-2 text-sm">{getFriendlyErrorMessage(error, 'There was an issue loading this product. Please try again later.')}</p></div></div>)
  if (!productAny) return (<div className="page-container flex items-center justify-center min-h-[40vh]"><div className="card p-6 sm:p-8 text-center max-w-md"><p className="text-gray-600 dark:text-gray-300 text-lg">This product could not be found</p><p className="text-gray-500 mt-2 text-sm">It may have been removed or is no longer available.</p></div></div>)

  return (
    <div className="page-container">
      <div className="grid md:grid-cols-2 gap-6 md:gap-12">
        <div className="w-full max-w-md mx-auto md:max-w-none">
          <div className="relative w-full aspect-[4/5] max-h-[min(70vh,420px)] md:max-h-none md:aspect-[3/4] bg-gray-100 dark:bg-gray-800 overflow-hidden rounded-xl md:rounded-2xl shadow-sm" onClick={() => setZoomed(!zoomed)}>
             <MediaImage src={productAny.images?.[activeImage]} alt={productAny.name || 'Product image'} fill transform={{ width: 900, aspect: '3:4' }} className={`object-cover object-center transition-transform duration-500 ease-out ${zoomed ? 'cursor-zoom-out' : 'cursor-zoom-in'}`} />
             <MagnifyingGlassPlusIcon className="absolute bottom-3 right-3 w-5 h-5 text-white/80 drop-shadow hidden sm:block" />
             {zoomed && (
               <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center animate-fade-in pointer-events-none">
                 <div className="pointer-events-auto relative animate-fade-in">
                   <button type="button" className="absolute -top-10 right-0 text-white/80 hover:text-white p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors" onClick={(e) => { e.stopPropagation(); setZoomed(false); }} aria-label="Close zoom">
                     <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                   </button>
                   <MediaImage src={productAny.images?.[activeImage]} alt={productAny.name || 'Product image'} width={1200} height={1600} transform={{ width: 1200, aspect: '3:4' }} className="max-w-full max-h-[85vh] object-contain rounded-xl shadow-2xl" />
                 </div>
               </div>
             )}
           </div>
          <div className="flex gap-2 mt-3 overflow-x-auto justify-center md:justify-start pb-1 -mx-1 px-1">
            {(productAny.images || []).map((img: string, i: number) => (
              <button key={i} type="button"                className={`relative w-14 h-16 sm:w-16 sm:h-20 shrink-0 border-2 overflow-hidden rounded-lg hover:scale-105 active:scale-95 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-teal dark:focus:ring-brand-orange focus:ring-offset-1 ${activeImage === i ? 'border-brand-teal dark:border-brand-orange' : 'border-gray-200 dark:border-gray-700'}`} onClick={() => setActiveImage(i)}>
        ...
        <div className="animate-slide-up text-center md:text-left">
          <p className="micro-label">{productAny.brand}</p>
          <h1 className="font-display text-2xl sm:text-3xl md:text-4xl font-semibold mt-2 tracking-tight">{productAny.name}</h1>
          <div className="flex items-center gap-2 mt-3 justify-center md:justify-start">
            <div className="flex gap-0.5">{[1, 2, 3, 4, 5].map((i) => <StarIcon key={i} className={`w-5 h-5 ${i <= Math.round(productAny.rating || 0) ? 'text-yellow-500' : 'text-gray-300 dark:text-gray-600'}`} />)}</div>
            <span className="text-sm text-gray-500 dark:text-gray-400">{productAny.rating || 0} ({productAny.reviewCount || 0} reviews)</span>
          </div>
          <div className="flex items-center gap-3 mt-4 justify-center md:justify-start flex-wrap">
            <span className="text-3xl sm:text-4xl font-bold text-brand-teal dark:text-brand-orange tracking-tight">{formatPrice(salePrice)}</span>
            {(productAny.discount || 0) > 0 && <><span className="text-base sm:text-lg text-gray-400 line-through">{formatPrice(productAny.price || 0)}</span><span className="text-sm font-semibold text-brand-teal dark:text-brand-orange bg-brand-teal/10 dark:bg-brand-orange/10 px-2.5 py-1 rounded-full">{productAny.discount > 100 ? `-${formatPrice(productAny.discount)}` : `-${productAny.discount}%`}</span></>}
          </div>
          <div className="mt-6">
            <h3 className="font-medium text-sm mb-3">Size</h3>
            <div className="flex flex-wrap gap-2 justify-center md:justify-start">
              {(productAny.sizes || []).map((size: string) => (
                 <button key={size} type="button" className={`min-w-[2.5rem] px-4 py-2.5 text-sm font-medium border rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-teal dark:focus:ring-brand-orange focus:ring-offset-1 ${selectedSize === size ? 'bg-brand-teal text-white dark:bg-brand-orange dark:text-gray-900 border-brand-teal dark:border-brand-orange shadow-sm shadow-brand-teal/20 dark:shadow-brand-orange/20' : 'border-gray-300 dark:border-gray-700 hover:border-brand-teal dark:hover:border-brand-orange text-gray-700 dark:text-gray-300 hover:text-brand-teal dark:hover:text-white'}`} onClick={() => setSelectedSize(size)}>{size}</button>
              ))}
            </div>
          ...
          <div className="mt-6">
            ...
            <div className="flex gap-2 justify-center md:justify-start flex-wrap">
              {productColors.map((color) => (
                <button
                  ...
                  type="button"
                   className={`w-9 h-9 rounded-full border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-teal dark:focus:ring-brand-orange focus:ring-offset-1 ${selectedColor === color.name ? 'border-brand-teal dark:border-white ring-2 ring-brand-teal/30 dark:ring-white/30 scale-110' : 'border-gray-300 dark:border-gray-600 hover:scale-105'}`}
           ...
           <div className="mt-6 rounded-xl border border-gray-200 dark:border-gray-700 px-4 py-3 text-left">
          ...
          <div className="flex flex-col items-center md:items-start gap-3 sm:gap-4 mt-6">
            <div>
              <p className="text-sm font-medium mb-2 text-center md:text-left">Quantity</p>
                 <div className="flex items-center border border-gray-300 dark:border-gray-700 w-fit rounded-xl overflow-hidden">
                 <button
                   type="button"
                   className="px-4 py-3 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                   disabled={quantity <= 1}
             ...
             <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full max-w-sm md:max-w-none">
               <button className="btn-primary flex-1 min-w-0 text-sm sm:text-base active:scale-[0.97] transition-transform shadow-lg shadow-brand-teal/25 dark:shadow-brand-orange/25 disabled:opacity-50 disabled:shadow-none" disabled={!selectedSize || !selectedColor || stockAvailable === 0 || maxSelectable === 0} onClick={handleAddToCart}>Add to Cart</button>
               <Link href="/checkout" className="btn-secondary flex-1 text-sm sm:text-base active:scale-[0.97] transition-transform text-center" aria-label="Buy now and proceed to checkout">Buy Now</Link>
               <button
                 type="button"
                 className="p-3 rounded-xl border border-gray-300 dark:border-gray-700 shrink-0 text-gray-500 dark:text-gray-400 hover:text-brand-teal dark:hover:text-brand-orange hover:border-brand-teal dark:hover:border-brand-orange transition-colors"
                 onClick={async () => {
                   if (!productAny) return
                   try {
          ...
          {productAny.description && (
            <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800">
              <h2 className="text-lg font-display font-semibold mb-3 text-gray-900 dark:text-white">Description</h2>
              <p className="text-gray-600 dark:text-gray-300 leading-relaxed text-sm sm:text-base">{productAny.description}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <section className="mt-12 sm:mt-16">
              ...
              <form className="space-y-4" onSubmit={submitReview}>
                <div>
                  ...
                  <div className="flex gap-1 justify-center md:justify-start">
                    {[1, 2, 3, 4, 5].map((i) => (
                       <button key={i} type="button" className="p-1 rounded-lg hover:bg-yellow-50 dark:hover:bg-yellow-900/20 transition-colors focus:outline-none focus:ring-2 focus:ring-brand-teal dark:focus:ring-brand-orange focus:ring-offset-1" onClick={() => setReviewRating(i)}>
                         {i <= reviewRating ? <StarIcon className="w-7 h-7 text-yellow-500" /> : <StarOutline className="w-7 h-7 text-gray-300 dark:text-gray-600" />}
                       </button>
                    ))}
                  </div>
                </div>
        ...
        {reviews.length ? reviews.map((review: any) => (
          <div key={review.id} className="card p-4 sm:p-5 mb-4">
            <p className="font-medium text-sm">{review.userName || 'Customer'}</p>
             <div className="flex mt-1">{[1, 2, 3, 4, 5].map((i) => <StarIcon key={i} className={`w-3.5 h-3.5 ${i <= (review.rating || 0) ? 'text-yellow-500' : 'text-gray-300 dark:text-gray-600'}`} />)}</div>
