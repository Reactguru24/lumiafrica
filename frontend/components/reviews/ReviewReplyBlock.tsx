'use client'

import { VendorVerificationBadge } from '@/components/vendor/VendorVerificationBadge'
import { resolveVendorBadge } from '@/lib/utils/vendorRating'

type ReviewReplyBlockProps = {
  reply: string
  label?: string
  vendorRating?: number
  verificationBadge?: string
}

export function ReviewReplyBlock({
  reply,
  label = 'Store reply',
  vendorRating,
  verificationBadge,
}: ReviewReplyBlockProps) {
  const badge = resolveVendorBadge(vendorRating, verificationBadge)

  return (
    <div className="mt-3 ml-0 sm:ml-4 pl-0 sm:pl-4 border-l-0 sm:border-l-2 border-brand-orange/40">
      <div className="bg-gray-50 dark:bg-gray-800/80 rounded-md p-3">
        <div className="flex items-center gap-1.5 mb-1">
          <p className="text-xs font-semibold text-brand-orange">{label}</p>
          <VendorVerificationBadge badge={badge} className="w-4 h-4" />
        </div>
        <p className="text-sm text-gray-700 dark:text-gray-300">{reply}</p>
      </div>
    </div>
  )
}
